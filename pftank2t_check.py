...
This code generates backstop load review outputs for checking the a Xija model.
It also generates model validation plots comparing predicted values to
telemetry for the previous three weeks.
"""
...
def main(opt):
    ...
    logger.info('##############################'
                '#######################################')
    logger.info('# pftank2t_check.py run at %s by %s'
                % (proc['run_time'], proc['run_user']))
    logger.info('# pftank2t_check version = {}'.format(VERSION))
...
def make_validation_plots(opt, tlm, db):
    ...
    pred = {'pftank2t': model.comp['pftank2t'].mvals,
